export default {
  nav: {
    tagline: 'The world, at a glance.',
    live: 'LIVE',
  },
  layers: {
    title: 'Layers',
    news:    'News',
    oil:     'Oil',
    weather: 'Weather',
  },
  filter: {
    label: 'Filter',
    all:            'All',
    politics:       'Politics',
    economy:        'Economy',
    technology:     'Technology',
    climate:        'Climate',
    culture:        'Culture',
    infrastructure: 'Infrastructure',
  },
  sidebar: {
    aria:           'Story details',
    aiSummary:      'AI Summary',
    readArticle:    'Read full article',
    close:          'Close',
    noSelection:    'Click a pin on the globe to explore a story.',
    timestamp:      'Published',
    source:         'Source',
    justNow:        'just now',
    minutesAgo:     '{n}m ago',
    hoursAgo:       '{n}h ago',
    sentiment: {
      positive: 'Positive',
      neutral:  'Neutral',
      negative: 'Negative',
    },
  },
  timeline: {
    aria:  'Timeline range',
    title: 'Timeline',
    label: 'Timeline',
    past:  '30 days ago',
    now:   'Now',
    live:  'Live · now',
  },
  globe: {
    hint: 'Activate a layer to explore world events.',
  },
  weather: {
    loading: 'Loading weather data.',
    empty:   'No weather data cached yet.',
    error:   'Weather data is unavailable right now.',
    searchLabel: 'Weather search',
    searchScope: 'Place',
    searchPlaceholder: 'City or country',
    searching: 'Searching.',
    searchError: 'Search is unavailable right now.',
    noMatch: 'No place found.',
  },
  cluster: {
    aria:     'Articles at this location',
    articles: 'articles at this location',
  },
  oil: {
    loading:      'Loading oil prices.',
    error:        'Oil price data unavailable.',
    noKey:        'Add EIA_API_KEY to backend/.env to enable live prices.',
    empty:        'No price data cached yet.',
    panelTitle:   'Crude Oil Prices',
    wti:          'WTI',
    brent:        'Brent',
    perBarrel:    '/ bbl',
    producer:     'Producer',
    consumer:     'Consumer',
    both:         'Producer & Consumer',
    production:   'Production',
    mbDay:        'mb/d',
    source:       'Source: EIA Open Data',
    chartLabel:   'Last 60 days (USD/bbl)',
    change:       'vs prev. day',
  },
}
